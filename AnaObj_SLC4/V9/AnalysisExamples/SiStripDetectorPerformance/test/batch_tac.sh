...
###############################################################################################################
DEFAULT_CASTOR_OUTPUT_DIRECTORY="/castor/cern.ch/user/g/giordano/TAC/TIB"
...
set_shell_variables(){
 ...
 DIR_WHERE_TO_EVAL="/afs/cern.ch/user/g/giordano/scratch0/CMSSW/Development/TAC/CMSSW_1_2_0_pre2"
...
# PEDESTAL_DIR="/afs/cern.ch/user/d/dkcira/scratch0/MTCC/2006_08_14_code_090/pedestals";
 ...
 CASTOR_DIR="/castor/cern.ch/cms/testbeam/TAC/TIB/run"
...
# PED_LOG_FILE="${MTCC_OUTPUT_DIR}/${RUNNR}_mtcc_pedestals.log";
 ...
 # templates
 TEMPLATE_CMSSW_CFG="${DIR_WHERE_TO_EVAL}/src/AnalysisExamples/SiStripDetectorPerformance/data/template_TAC_processing.cfg"
...
#    DQM_OUTPUT_FILE="${MTCC_OUTPUT_DIR}/${RUNNR}_dqm_${rfile}.root"
    ...
    touch $CMSSW_CFG;
    cat  "$TEMPLATE_CMSSW_CFG" | sed "s@OUTFILE_CLUSTER@${POOL_OUTPUT_FILE}@" | sed "s@SCRIPT_DQM_OUTPUT_FILE@${DQM_OUTPUT_FILE}@" | sed "s@insert_input_file_list@${FILE_FULL_PATH}@" >>  ${CMSSW_CFG}
  done
}
