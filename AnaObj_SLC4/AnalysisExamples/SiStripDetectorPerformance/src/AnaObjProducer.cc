...
void AnaObjProducer::produce(edm::Event& e, const edm::EventSetup& es) {
  ...
  if (runnumber!=e.id().run()) // do it only once for each run
    {
      ...
      // these are the IOV's by hand...
      // Ask Andrea for his good tickmark run list

      if(runnumber>=6095 && runnumber<6448){
	inputfile = "tickmarks_dump_run_6094.txt";
      }else if(runnumber>=6448 && runnumber<6835){
	inputfile = "tickmarks_dump_run_6445.txt";
      }else if(runnumber>=6835 && runnumber<7233){
	inputfile = "tickmarks_dump_run_6891.txt";
      }else if(runnumber>=7233 && runnumber<8433){
	inputfile = "tickmarks_dump_run_7222.txt";
      }else if(runnumber>=8433 && runnumber<9149){
	inputfile = "tickmarks_dump_run_8427.txt";
      }else if(runnumber>=9149 && runnumber<9244){
	inputfile = "tickmarks_dump_run_9148.txt";
      }else if(runnumber>=9244 && runnumber<10213){
	inputfile = "tickmarks_dump_run_9242.txt";
      }else if(runnumber>=10213 && runnumber<10610){
	inputfile = "tickmarks_dump_run_10212.txt";
      }else if(runnumber>=10610 && runnumber<10828){
	inputfile = "tickmarks_dump_run_10607.txt";
      }else if(runnumber>=10828 && runnumber<11178){
	inputfile = "tickmarks_dump_run_10821.txt";
      }else if(runnumber>=11178 && runnumber<11315){
	inputfile ="tickmarks_dump_run_11176.txt";
      }else if(runnumber>=11315 && runnumber<12044){
	inputfile ="tickmarks_dump_run_11311.txt";
      }else if(runnumber>=12044){
	inputfile= "tickmarks_dump_run_12043.txt";
      }
      ...
      int module;
      double fiber0, fiber1, fiber2;

      if (in){

	while (! in.eof() ){
	  in >> module >> fiber0 >> fiber1 >> fiber2 ;

	  v_detid.push_back(module);
	  v_fiber0.push_back(fiber0);
	  v_fiber1.push_back(fiber1);
	  v_fiber2.push_back(fiber2);

	  if(fiber1 == -1){
	    v_apvnum.push_back(2);
	  }else{
	    v_apvnum.push_back(3);
	  }
	}
      }
    }
  ...
  for( oDSVclusterInfoIter = oDSVClusterInfos->begin();
	...
	if(rawId == v_detid[i]){
	  if(v_apvnum[i]==2){
	    if(thisapv ==0 && v_fiber0[i] !=0 ) {clusterchg_tickcorr = clusterchg * (640./v_fiber0[i]); clustertick = v_fiber0[i];}
	    if(thisapv ==1 && v_fiber2[i] !=0 ) {clusterchg_tickcorr = clusterchg * (640./v_fiber2[i]); clustertick = v_fiber2[i];}
	  }else{
	    if(thisapv ==0 && v_fiber0[i] !=0 ) {clusterchg_tickcorr = clusterchg * (640./v_fiber0[i]); clustertick = v_fiber0[i];}
	    if(thisapv ==1 && v_fiber1[i] !=0 ) {clusterchg_tickcorr = clusterchg * (640./v_fiber1[i]); clustertick = v_fiber1[i];}
	    if(thisapv ==2 && v_fiber2[i] !=0 ) {clusterchg_tickcorr = clusterchg * (640./v_fiber2[i]); clustertick = v_fiber2[i];}
	  }
	  break;
	}
      }
	...
	if (ClusIter->firstStrip() == oIter->firstStrip()){
      ...
      analyzedCluster.clusterchg_tickcorr = clusterchg_tickcorr;
      analyzedCluster.clustertick      = clustertick;
  ...
  for(int AnaCluIter=0; AnaCluIter!=numAnaClu; ++AnaCluIter){
    ...
    clusterchg_tickcorr = tmpAnaClu.clusterchg_tickcorr;
    clustertick      = tmpAnaClu.clustertick;
