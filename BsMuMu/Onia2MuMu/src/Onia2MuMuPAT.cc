		...
		for(View<pat::Muon>::const_iterator it2 = it+1; it2 != itend;++it2){
			...
			if (it->track().isNonnull() && it2->track().isNonnull()) {
				...
				if (myVertex.isValid()) {
					...
					pat::Muon mu1(*it);
					math::XYZTLorentzVector m1p4(fromPtEtaPhiToPxPyPz(myVertex.refittedTrack(t_tks[0]).track().pt(),
							myVertex.refittedTrack(t_tks[0]).track().eta(),
							myVertex.refittedTrack(t_tks[0]).track().phi(),
							muMasses[0]));
					mu1.setP4(m1p4);
					pat::Muon mu2(*it2);
					math::XYZTLorentzVector m2p4(fromPtEtaPhiToPxPyPz(myVertex.refittedTrack(t_tks[0]).track().pt(),
							myVertex.refittedTrack(t_tks[0]).track().eta(),
							myVertex.refittedTrack(t_tks[0]).track().phi(),
							muMasses[0]));
					mu2.setP4(m2p4);
