...
gotoVertexFound:
	    ...
	    if (rmu1 != 0 && (deltaR(rmu1->innerTrack()->eta(),rmu1->innerTrack()->phi(),tk->eta(),tk->phi())< 1.e-5)) continue;
	    if (rmu2 != 0 && (deltaR(rmu2->innerTrack()->eta(),rmu2->innerTrack()->phi(),tk->eta(),tk->phi())< 1.e-5)) continue;
