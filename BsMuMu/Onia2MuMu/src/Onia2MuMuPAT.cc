				 ...
				 const reco::TransientTrack & refittedTrack1, const reco::TransientTrack & refittedTrack2)
{
  ...
  int highPurity1 = 0;
  if(refittedTrack1.track().quality(reco::TrackBase::highPurity)) myCand.addUserInt("highPurity1", 1);
  myCand.addUserInt("highPurity1", highPurity1);
  myCand.addDaughter(mu2, "muon2");
  int highPurity2 = 0;
  if(refittedTrack2.track().quality(reco::TrackBase::highPurity)) myCand.addUserInt("highPurity2", 2);
...
{  
  ...
  for(View<pat::Muon>::const_iterator it = muons->begin(), itend = muons->end(); it != itend; ++it){
	...
	if (myVertex.isValid()) {
	  ...
	  TVector3 pCand(jpsi.px(), jpsi.py(), jpsi.pz());
