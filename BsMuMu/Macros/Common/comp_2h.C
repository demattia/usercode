...
void comp_2h(const TString & parallelAnalysisFile = "Barrel_preselection.root",
	  const TString & mainAnalysisFile = "data_main_barrel.root", bool MC = true)
{
  ...
  for( std::vector<Vars>::const_iterator it = vars.begin(); it != vars.end(); ++it ) {
    ...
    hB->Sumw2();
    if(hA) {   
      if(MC) hA->Scale(1./hA->Integral()); 
      else hA->Scale(1.);
    }
    if(hB) {
      if(MC) hB->Scale(1./hB->Integral());
      else hB->Scale(1.);
      //else hB->Scale(1. * hA->Integral()/hB->Integral());
    }
