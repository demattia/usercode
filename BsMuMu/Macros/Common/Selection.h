...
TString Selection(const bool endcaps, const bool data, const bool cut_based = false, const bool blinding = false, const int splitting = 0, const TString & maxRun = "") {
  ...
  // Preselection cuts
  TString preselection("(m > 4.9 && m < 5.9 && me < 0.2 && pt > 5. && pt < 9999. && m1pt > 4. && m1pt < 999. && m2pt > 4. && m2pt < 999. && fl3d < 2. && fls3d > 0. && fls3d < 200. && chi2dof < 10. && pvip < 0.1 && pvips < 5. && maxdoca < 0.1 && acos(cosAlpha3D) < 0.3 && closetrk < 21 && docatrk < 0.25 && iso > 0. && (mu1_charge*mu2_charge == -1) && (lxysig > 3) && (abs(pvlip) < 1.0) && (abs(pvlip)/abs(pvlipErr) < 5.0))");
  cuts += " && " + preselection;
  ...
  if( endcaps ) {
    TString endcapsCuts("!"+barrelCuts+" && m1eta < 2.4 && m2eta < 2.4 && m1eta > -2.4 && m2eta > -2.4");
    cuts += " && " + endcapsCuts;
  }
  else {
    cuts += " && " + barrelCuts;
