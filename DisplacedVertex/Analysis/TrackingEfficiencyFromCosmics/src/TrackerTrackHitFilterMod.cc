...
namespace reco { namespace modules {
  class TrackerTrackHitFilterMod : public edm::EDProducer {
  public:
    ...
    void produceFromTrajectory( const edm::EventSetup &iSetup, const Trajectory *itt, std::vector<TrackingRecHit *>&hits, bool & affected);
    void produceFromTrack( const edm::EventSetup &iSetup, const Track *itt, std::vector<TrackingRecHit *>&hits, bool & affected);
  private:
    class Rule {
    public:
...
TrackerTrackHitFilterMod::Rule::Rule(const std::string &str) {
    ...
    if (!regex_match(str.c_str(), match, rule)) {
      ...
      std::cout<<"*** Rule Command given to TrackerTrackHitFilterMod:\t"<<str<<std::endl;
...
int TrackerTrackHitFilterMod::Rule::layer(DetId detid) const {
...
TrackerTrackHitFilterMod::TrackerTrackHitFilterMod(const edm::ParameterSet &iConfig) :
  ...
  saveOnlyAffectedTracks_( iConfig.getUntrackedParameter<bool>("SaveOnlyAffectedTracks"))
{
    // sanity check
    ...
    if(rejectLowAngleHits_ && !useTrajectories_){
      throw cms::Exception("Configuration") << "Wrong configuration of TrackerTrackHitFilterMod. You cannot apply the cut on the track angle without using Trajectories!\n";
    }
    if (!useTrajectories_ && PXLcorrClusChargeCut_>0){
      throw cms::Exception("Configuration") << "Wrong configuration of TrackerTrackHitFilterMod. You cannot apply the cut on the corrected pixel cluster charge without using Trajectories!\n";
    }

    if(pxlTPLqBin_.size()>2){
      std::cout<<"Warning from TrackerTrackHitFilterMod: vector with qBin cuts has size > 2. Additional items will be ignored."<<std::endl;
    }
...
void
TrackerTrackHitFilterMod::produce(edm::Event &iEvent, const edm::EventSetup &iSetup)
{
  //Dump Run and Event
  iRun=iEvent.id().run();
  ...
  if(useTrajectories_){
    for (TrajTrackAssociationCollection::const_iterator itass = assoMap->begin();  itass != assoMap->end(); ++itass){
      ...
      bool affected = false;
      produceFromTrajectory(iSetup,myTrajectory,hits,affected);

      // When saving only affected tracks skip this track if its hits are unchanged
      if( (!affected) && saveOnlyAffectedTracks_ ) continue;
    ...
    for (std::vector<reco::Track>::const_iterator ittrk = tracks->begin(), edtrk = tracks->end(); ittrk != edtrk; ++ittrk) {
      ...
      bool affected = false;
      produceFromTrack(iSetup,trk,hits,affected);
...
TrackCandidate
TrackerTrackHitFilterMod::makeCandidate(const reco::Track &tk, std::vector<TrackingRecHit *>::iterator hitsBegin, std::vector<TrackingRecHit *>::iterator hitsEnd) {
...
void TrackerTrackHitFilterMod::produceFromTrack(const edm::EventSetup &iSetup, const Track *itt , std::vector<TrackingRecHit *>&hits, bool & affected)
{
  // loop on tracks
  hits.clear(); // extra safety
  std::cout << "This track has " << std::distance(itt->recHitsBegin(),itt->recHitsEnd()) << " recHits" << std::endl;
  ...
  for (trackingRecHit_iterator ith = itt->recHitsBegin(), edh = itt->recHitsEnd(); ith != edh; ++ith) {
    ...
    if (verdict == 0) {
      ...
      affected = true;
      std::cout << "Removing the hit from this track" << std::endl;
  ...
  std::cout << "After removal has " << hits.size() << " recHits" << std::endl;
}//end TrackerTrackHitFilterMod::produceFromTrack


void TrackerTrackHitFilterMod::produceFromTrajectory(const edm::EventSetup &iSetup, const Trajectory *itt, std::vector<TrackingRecHit *>&hits, bool & affected){
  hits.clear(); // extra safety
  nOverlaps=0;
...
bool TrackerTrackHitFilterMod::checkStoN(const edm::EventSetup &iSetup, const DetId &id, const TrackingRecHit* therechit){
      ...
      if( subdetStoN_[subdet_cnt-1]){//check that hit is in a det belonging to a subdet where we decided to apply a S/N cut
        ...
        if (type == typeid(SiStripRecHit2D)) {
          ...
          if (hit!=0) 	 cluster = &*(hit->cluster());
            ...
            edm::LogError("TrackerTrackHitFilterMod")<< "TrackerTrackHitFilterMod::checkStoN : Unknown valid tracker hit in subdet " << id.subdetId()<< "(detID="<<id.rawId()<<")\n ";
            keepthishit = false;
          }
        }
...
bool TrackerTrackHitFilterMod::checkHitAngle(const TrajectoryMeasurement &meas){
  ...
  /*
  edm::LogDebug("TrackerTrackHitFilterMod")<<"TSOS parameters: ";
  edm::LogDebug("TrackerTrackHitFilterMod") <<"Global momentum: "<<tsos.globalMomentum().x()<<"  "<<tsos.globalMomentum().y()<<"  "<<tsos.globalMomentum().z();
  edm::LogDebug("TrackerTrackHitFilterMod") <<"Local momentum: "<<tsos.localMomentum().x()<<"  "<<tsos.localMomentum().y()<<"  "<<tsos.localMomentum().z();
...
int TrackerTrackHitFilterMod::layerFromId (const DetId& id) const
{
...
using reco::modules::TrackerTrackHitFilterMod;
DEFINE_FWK_MODULE(TrackerTrackHitFilterMod);
