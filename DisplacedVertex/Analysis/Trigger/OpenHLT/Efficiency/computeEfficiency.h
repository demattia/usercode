...
#ifndef computeEfficiency_h
#define computeEfficiency_h
...
public :
   ...
   computeEfficiency(TTree *tree=0);
   computeEfficiency(const TString & fileName, const TString & outputFileName,
                     const double & startingParallelCut,
                     const double & ptMin = 20., const double & ptMax = 30.);
   virtual ~computeEfficiency();
...
#ifdef computeEfficiency_cxx
computeEfficiency::computeEfficiency(TTree *tree)
{
...
computeEfficiency::computeEfficiency(const TString & fileName, const TString & outputFileName,
                                     const double & startingParallelCut,
                                     const double & ptMin, const double & ptMax)
  : outputFileName_(outputFileName),
    startingParallelCut_(startingParallelCut),
    ptMin_(ptMin),
    ptMax_(ptMax)
{
  TTree * tree = 0;
...
computeEfficiency::~computeEfficiency()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t computeEfficiency::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t computeEfficiency::LoadTree(Long64_t entry)
{
...
void computeEfficiency::Init(TTree *tree)
{
...
Bool_t computeEfficiency::Notify()
{
...
void computeEfficiency::Show(Long64_t entry)
{
...
Int_t computeEfficiency::Cut(Long64_t entry)
{
...
#endif // #ifdef computeEfficiency_cxx
