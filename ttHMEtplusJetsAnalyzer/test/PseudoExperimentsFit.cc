...
void fit(TString  inputFileNameSignal,            // input file name for signal
         TString  inputFileNameBackground,        // input file name for background
	 TString  sgHisto,                        // input background template name
	 TString  bkHisto,                        // input signal template name
  ...
  TFile *inputFileSignal = new TFile(inputFileNameSignal);
  TFile *inputFileBackground = new TFile(inputFileNameBackground);

  TDirectory * dirSignal = (TDirectory*) inputFileSignal->Get("EventVariables_2tags");
  TDirectory * dirBackground = (TDirectory*) inputFileBackground->Get("EventVariables_2tags");

  backgroundHisto = (TH1D*)dirBackground->Get(bkHisto);  //global pointer used in ftotal
  signalHisto     = (TH1D*)dirSignal->Get(sgHisto);      //global pointer used in ftotal

  cout << "histograms taken background = " << dirBackground->Get(bkHisto) << endl;
  cout << "histograms taken signal = " << signalHisto << endl;
