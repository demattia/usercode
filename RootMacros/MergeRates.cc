...
void hadd() {
  ...
  Target = TFile::Open( "QCD.root", "RECREATE" );

  FileList = new TList();

  // ************************************************************
  // List of Files
  FileList->Add( TFile::Open("Ana_QCD_30-50_tk3.root") );    // 1
  FileList->Add( TFile::Open("Ana_QCD_50-80_tk3.root") );    // 2
  FileList->Add( TFile::Open("Ana_QCD_80-120_tk3.root") );   // 3
  FileList->Add( TFile::Open("Ana_QCD_120-170_tk3.root") );  // 4
  FileList->Add( TFile::Open("Ana_QCD_170-230_tk3.root") );  // 5
  FileList->Add( TFile::Open("Ana_QCD_230-300_tk3.root") );  // 6
  FileList->Add( TFile::Open("Ana_QCD_300-380_tk3.root") );  // 7

  // List of Cross-sections (in pb) multiplied by the high luminosity factor
  double crossSections[7] = { 163000000.*0.01,
			      21600000.*0.01,
			      3080000.*0.01,
			      494000.*0.01,
			      101000.*0.01,
