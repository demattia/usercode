...
to the grid with crab. You must first initialize your grid certificate.
...
print "*******************************************************************************************"
print "* Preparing to create and submit jobs. Bu sure to have initialized your grid certificate. *"
print "*******************************************************************************************"

print 'cfgFileName =',
if len(sys.argv) != 2:

    print "No cfg name specified, using default LaunchCrab.cfg"
    print
    f=open('LaunchCrab.cfg')
else:
    print "Using cfg",
    print sys.argv[1]
    print
...
for line in f:
    ...
    if not(line.startswith("#")) and line != "\n":
        ...
        if varName == 'storageElement':
            storageElement = varValue.split(',')
        if varName == 'storagePath':
            storagePath = varValue.split(',')
...
if (not(len(workingDir) == typeNum and len(totalNumberOfEvents) == typeNum and len(eventsPerJob) == typeNum and len(storageElement) == typeNum and len(storagePath) == typeNum)):
...
for type in range(typeNum):
    ...
    for i in range(jobsNum):
        ...
        crabFileName = workingDir[type].strip() + "/" + "crab_" + str(i) + ".cfg"
        crabFile = open(crabFileName, 'w')
        for s in open('crab.cfg'):
            if( not(s.startswith("#")) ):
                # Set the cfgFile name with full path (the path should not be needed, but use it for safety)
                if( s.find("pset") != -1 ):
                    temp = s.split('=')
                    temp = temp[1].strip()
                    ...
                    crabFile.write(s.replace(temp, currentDir.strip() + "/" + workingDir[type].strip() + "/"+ tempCfgFileName+'_'+str(i)+'.cfg'))
                # Number of events
                elif( s.find("total_number_of_events") != -1 ):
                    temp = s.split("=")
                    temp = temp[1].strip()
                    print "temp = " + temp
                    print "eventsPerJob["+str(type)+"] = "+eventsPerJob[type]
                    crabFile.write(s.replace( temp, str(eventsPerJob[type]) ))
                # Set the output file name
                elif( s.find("output_file") != -1 ):
                    temp = s.split("=")
                    temp = temp[1].strip()
                    tempFileName = outFileName[type].split('.')
                    crabFile.write(s.replace(temp, tempFileName[0].strip()+"_"+str(i)+".root"))
                elif( s.find("storage_element") != -1 ):
                    temp = s.split("=")
                    temp = temp[1].strip()
                    crabFile.write(s.replace(temp, storageElement[type]))
                elif( s.find("storage_path") != -1 ):
                    temp = s.split("=")
                    temp = temp[1].strip()
...
print
print "The crab jobs will fill your home dir with LSF* directories containing stdout and stderr files."
