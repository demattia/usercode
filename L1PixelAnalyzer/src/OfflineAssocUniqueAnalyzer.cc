...
#include "AnalysisExamples/L1PixelAnalyzer/interface/OfflineAssocUniqueAnalyzer.h"
...
L1Trig OfflineAssocUniqueAnalyzer::L1Trigger;

//
// constructors and destructor
//
OfflineAssocUniqueAnalyzer::OfflineAssocUniqueAnalyzer(const edm::ParameterSet& iConfig) :
  conf_( iConfig ),
  ...
  OutputEffFileName( iConfig.getUntrackedParameter<string>( "OutputEffFileName" ) )
{
  ...
  int SizeEt3 = 10;
  int SizeEt4 = 10;
  EffMultijetPixelSizeEt1_ = SizeEt1*dRindexSize;
  EffMultijetPixelSizeEt2_ = SizeEt2*dRindexSize;
  EffMultijetPixelSizeEt3_ = SizeEt3*dRindexSize;
  EffMultijetPixelSizeEt4_ = SizeEt4*dRindexSize;
  EffMultijetPixelSize_ = SizeEt1*SizeEt2*SizeEt3*SizeEt4*dzIndexSize*dRindexSize;
  offlineEffMultijetPixelSize_ = EffMultijetPixelSize_;

  EffMEtJetPixelSize_ = 100*dzIndexSize*dRindexSize;
  ...
  Multi_Assoc_DR_ = new MultiTH1F( "AssocDR", "Number of associated pixelJets", 10, 0, 10, 10, 0.1, 2.1, OutputFile ); 
...
OfflineAssocUniqueAnalyzer::~OfflineAssocUniqueAnalyzer()
{
...
void
OfflineAssocUniqueAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace std;
  ...
  for( int numdz = 0; numdz < bins_; ++numdz ) {
    ...
    if ( numVertex > 0 ) {
      ...
      if ( numVertex > 1 ) {
        ...
        int num = 0;
        for ( ; svec_z_it != vec_vertex_aptr->end()-1; ++svec_z_it, ++num ) {
  ...
  // Create a new collection with cen+tau jets
  vector<SimpleJet> vecCenTauJets( vec_TriggerCenJet );
  vector<SimpleJet>::const_iterator tau_it = vec_TriggerTauJet.begin();
  for ( ; tau_it != vec_TriggerTauJet.end(); ++tau_it ) {
    vecCenTauJets.push_back( *tau_it );
    //         vecTauPtr.push_back( &(*tau_it) );
  }
  sort( vecCenTauJets.begin(), vecCenTauJets.end() );
  reverse( vecCenTauJets.begin(), vecCenTauJets.end() );
  ...
  for ( int PVnum=0; PVnum<PVlastNum; ++PVnum ) {
    ...
    for ( int dzIndex=dzIndexMin; dzIndex<dzIndexMax; ++dzIndex ) {
      ...
      // Take the pixelJets from the primary vertex
      auto_ptr<vector<Vertex<SimplePixelJet> > > vec_vertex_aptr( PixeltrigMulti.VevVertex() );
      ...
      // Initialize here to count correctly the iterations on dR
      int et1Count = 0;
      int et2Count = 0;
      int et3Count = 0;
      int et4Count = 0;
      ...
      for ( int dRiter=0; dRiter<10; ++dRiter ) {
        float dR = float(dRiter+1)*0.2;

        // Define the association cone
        Associator<SimplePixelJet, SimpleJet> associator( dR );

        // Associate pixelJets to cen+tau jets
        std::auto_ptr<std::map<const SimplePixelJet*, const SimpleJet*> > assocMapCenTau( associator.Associate( PVpixelJets, vecCenTauJets ) );

        Multi_Assoc_DR_->Fill( assocMapCenTau->size(), dRiter );

        // Construct new collections containing only the CenJets and the TauJets associated to pixeljets
        vector<SimpleJet> vecAssocCenJet;
        vector<SimpleJet> vecAssocTauJet;
        std::map<const SimplePixelJet*, const SimpleJet*>::const_iterator assocCenTau_it = assocMapCenTau->begin();
        for ( ; assocCenTau_it != assocMapCenTau->end(); ++assocCenTau_it ) {
          SimpleJet assocJet( *(assocCenTau_it->second) );
          // If it was a central jet
          for ( vector<SimpleJet>::const_iterator cenIt = vec_TriggerCenJet.begin(); cenIt != vec_TriggerCenJet.end(); ++cenIt ) {
            if ( ( cenIt->pt() == assocJet.pt() && cenIt->eta() == assocJet.eta() && cenIt->phi() == assocJet.phi() ) || singleColl_ ) {
              vecAssocCenJet.push_back( *(assocCenTau_it->second) );
            }
          }
          for ( vector<SimpleJet>::const_iterator tauIt = vec_TriggerTauJet.begin(); tauIt != vec_TriggerTauJet.end(); ++tauIt ) {
            if ( ( tauIt->pt() == assocJet.pt() && tauIt->eta() == assocJet.eta() && tauIt->phi() == assocJet.phi() ) && !singleColl_ ) {
              vecAssocTauJet.push_back( *(assocCenTau_it->second) );
            }
          }
        }
        ...
        int mEtJetCount = 0;
        //        for ( int dzIndex=dzIndexMin; dzIndex<dzIndexMax; ++dzIndex ) {
        L1PixelTrig<SimplePixelJet> PixeltrigMEtJet(3+PVnum, float(dzIndex)*0.1, numTkCut_);
...
void OfflineAssocUniqueAnalyzer::beginJob(const edm::EventSetup&) {
}

// void OfflineAssocUniqueAnalyzer::meanHistoSetup( TH1F * meanhisto_ptr, vector<TH1F *> vec_histo ) {
...
void OfflineAssocUniqueAnalyzer::endJob() {
...
DEFINE_FWK_MODULE(OfflineAssocUniqueAnalyzer);
