...
//      run TDAnaProducer, run Smooth.C on rootple, run TDAnaProducer again with
...
#include "AnalysisExamples/L1PixelAnalyzer/interface/TDAnaProducer.h"
...
L1Trig TDAnaProducer::L1Trigger;

// Constructors and destructor
// ---------------------------
TDAnaProducer::TDAnaProducer(const edm::ParameterSet& iConfig) :
  conf_( iConfig ),
  ...
  TDAnaObject_( iConfig.getParameter<string>( "TDAnaObject" ) )
{
  // Object to write in the event
  produces<anaobj::TDAnaObject>( TDAnaObject_ );
...
// ----------------------------------------------------------
void TDAnaProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  using namespace std;
  ...
  for ( GlobalMuonCollection::const_iterator muon = globalMuons->begin(); 
	...
	if ( cal->et()>25. ) { 
	  double d_eta = cal->eta()-muon->eta();
	  double d_phi = 3.1415926-fabs(fabs(cal->phi()-muon->phi())-3.1415926);
	  double dr2 = d_eta*d_eta + d_phi*d_phi;
	  if ( dr2<dr2min ) dr2min=dr2;
	}
      }
      if ( dr2min>0.25 ) muonevent = true;
    }
  }
  ...
  for ( SimpleElectronCollection::const_iterator elec = simpleElectrons->begin(); 
	...
	if ( cal->et()>25. && cal->emEnergyFraction()<0.95 ) { 
	  double d_eta = cal->eta()-elec->eta();
	  double d_phi = 3.1415926-fabs(fabs(cal->phi()-elec->phi())-3.1415926);
	  double dr2 = d_eta*d_eta + d_phi*d_phi;
	  if ( dr2<dr2min ) dr2min=dr2;
	}
      }
      if ( dr2min>0.25 ) elecevent = true;
    }
  }
  ...
  if ( caloJets->size() != 0 ) {
	...
	if ( PTOT>0 ) {
	  ...
	  if ( response && NJetsCut ) {
        ...
        // fill object //
        // ----------- //
        // Because of root genreflex compilation problem, cannot put all the
        // variables in the constructor and must use external methods.
        std::auto_ptr<TDAnaObject> TDAnaObjectPtr( new TDAnaObject( c8, m8, c6, m6,
                                                                    met, metsig, CorrSumEt, GoodHt,
                                                                    hbestcomb, chi2, mbbnohmax, dpbbnohmax,
                                                                    sumhed4, sumhed6, dpmin, dp1st,
                                                                    dp2nd, m_others, Et6, scprodthbest,
                                                                    thdetabest, m5, tbestcomb, wbestcomb,
                                                                    ttms1, metsignew ) );
        //                                                                     goodIc5Jets, UncorrHt,
        //                                                                     CorrHt, GoodHt2, UncorrSumEt, GoodSumEt,
        //                                                                     UncorrMEtSig, CorrMEtSig, m45best, chi2ext,
        //                                                                     sumet, metphi, dp12, dpbb,
        //                                                                     m45bestall, chi2extall, dpbball, mwmin,
        //                                                                     drpairbestall, m3a, mwa, ttms2,
        //                                                                     ttms3, sumhpd4, sumhpd6, dp3rd,
        //                                                                     PTOT, PTOTE2 ) );
        TDAnaObjectPtr->setGoodIc5Jets( goodIc5Jets );
        TDAnaObjectPtr->setUncorrHt( UncorrHt);
        TDAnaObjectPtr->setCorrHt( CorrHt );
        TDAnaObjectPtr->setGoodHt2( GoodHt2 );
        TDAnaObjectPtr->setUncorrSumEt( UncorrSumEt );
        TDAnaObjectPtr->setGoodSumEt( GoodSumEt );
        TDAnaObjectPtr->setUncorrMEtSig( UncorrMEtSig );
        TDAnaObjectPtr->setCorrMEtSig( CorrMEtSig );
        TDAnaObjectPtr->setM45best( m45best );
        TDAnaObjectPtr->setChi2ext( chi2ext );
        TDAnaObjectPtr->setSumet( sumet );
        TDAnaObjectPtr->setMetphi( metphi );
        TDAnaObjectPtr->setDp12( dp12 );
        TDAnaObjectPtr->setDpbb( dpbb );
        TDAnaObjectPtr->setM45bestall( m45bestall );
        TDAnaObjectPtr->setChi2extall( chi2extall );
        TDAnaObjectPtr->setDpbball( dpbball );
        TDAnaObjectPtr->setMwmin( mwmin );
        TDAnaObjectPtr->setDrpairbestall( drpairbestall );
        TDAnaObjectPtr->setM3a( m3a );
        TDAnaObjectPtr->setMwa( mwa );
        TDAnaObjectPtr->setTtms2( ttms2 );
        TDAnaObjectPtr->setTtms3( ttms3 );
        TDAnaObjectPtr->setSumhpd4( sumhpd4 );
        TDAnaObjectPtr->setSumhpd6( sumhpd6 );
        TDAnaObjectPtr->setDp3rd( dp3rd );
        TDAnaObjectPtr->setPtot( PTOT );
        TDAnaObjectPtr->setPtotE2( PTOTE2 );
        ...
        iEvent.put( TDAnaObjectPtr, TDAnaObject_ );
...
//      std::auto_ptr<TDAnaObject> TDAnaObjectPtr( new TDAnaObject() );
  ...
  if ( itdecay>=0 && itdecay<10 ) {
	...
	if ( NHEM>=3 ) {
	  Ntt[3][itdecay]++;
	  if ( elecevent ) Nelec[itdecay]++;
	  if ( muonevent ) Nmuon[itdecay]++;
	  if ( NHEM>=4 ) {
	    Ntt[4][itdecay]++;
	  }
	}
...
// -------------------------------------------------------------------------
void TDAnaProducer::beginJob(const edm::EventSetup&) {
}


//       method called once each job just after ending the event loop 
// -------------------------------------------------------------------------
void TDAnaProducer::endJob() {
...
// ------------------------
DEFINE_FWK_MODULE(TDAnaProducer);
