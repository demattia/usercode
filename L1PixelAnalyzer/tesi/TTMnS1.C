void TTMnS1 () {
  ...
  TH1D * H = dynamic_cast<TH1D*> (QCD[0]->Get("MTS1_0"));
  ...
  for ( int i=0; i<8; i++ ) {
    char Ename[20];
    char Pname[20];
    sprintf ( Ename, "MTS1_%d", i );
    sprintf ( Pname, "MNS1_%d", i );
  ...
  for ( int iN=0; iN<8; iN++ ) {
    char nameT[20];
    char nameN[20];
    sprintf ( nameT, "MTS1_%d", iN );
    sprintf ( nameN, "MNS1_%d", iN );
    double tot1[nbins]={0.};
    ...
    if ( iN==7 ) {  // Fix split histogram for Nt=9 && >9
      sprintf ( nameT, "MTS1_%d", iN+1 );
      sprintf ( nameN, "MNS1_%d", iN+1);
      for ( int i=0; i<8; i++ ) {
	MTtmp = dynamic_cast<TH1D*>(QCD[i]->Get(nameT));
	MNtmp = dynamic_cast<TH1D*>(QCD[i]->Get(nameN));
	for ( int ibin=1; ibin<=nbins; ibin++ ) {
