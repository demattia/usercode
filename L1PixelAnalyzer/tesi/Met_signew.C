{
  ...
  double es[100]={0.};
  double s_es[100]={0.};
  double IS=MEtSigNewS->Integral();
  double IScurr=IS;
  double maxS=0;
  for ( int ibin=1; ibin<=100; ibin++ ) {
    es[ibin-1] = IScurr/IS;
    s_es[ibin-1] = sqrt(es[ibin-1]*(1-es[ibin-1])/IS);
    IScurr -= MEtSigNewS->GetBinContent(ibin);
    ES_->SetBinContent(ibin,es[ibin-1]);
    ES_->SetBinError(ibin,s_es[ibin-1]);
    S_->SetBinContent(ibin,MEtSigNewS->GetBinContent(ibin)/IS);
    S_->SetBinError(ibin,MEtSigNewS->GetBinError(ibin)/IS);
    if ( MEtSigNewS->GetBinContent(ibin)/IS>maxS ) maxS=MEtSigNewS->GetBinContent(ibin)/IS;
  }
  ...
  for ( int i=0; i<8; i++ ) {
    QCD[i]->cd();
    for ( int ibin=1; ibin<=100; ibin++ ) {
      B[ibin-1]+=QCDxs[i]/N[i]*MEtSigNewS->GetBinContent(ibin);
      s2_B[ibin-1]+=(QCDxs[i]/N[i])*(QCDxs[i]/N[i])*MEtSigNewS->GetBinContent(ibin);
      IB += QCDxs[i]/N[i]*MEtSigNewS->GetBinContent(ibin);
      s2_IB += (QCDxs[i]/N[i])*(QCDxs[i]/N[i])*MEtSigNewS->GetBinContent(ibin);
    }
  }
