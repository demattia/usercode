void Smooth (TString sel) 
{
  ...
  TString var[11] = { "C8", "M45bestall", "Chi2extall", "MEtSig", "M8", "MEt", "DP12", "MEtDP2", "C6", "M6", "CorrSumEt" };
  TString pippo[11];
  TString pippotot[11];
  TString pippotth[11];
  TString pippototS[11];
  TString pippotthS[11];
  for ( int i=0; i<11; i++ ) { 
    pippo[i] = var[i]+sel; 
  ...
  TCanvas * b = new TCanvas ("b", "Kinematics comparison", 600, 600 );
  b->Divide(3,4);
