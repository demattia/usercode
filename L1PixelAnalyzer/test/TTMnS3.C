void TTMnS3 () {
  ...
  TH1D * H = dynamic_cast<TH1D*> (QCD[0]->Get("MTS3_0"));
  ...
  for ( int i=0; i<8; i++ ) {
    char Ename[20];
    char Pname[20];
    sprintf ( Ename, "MTS3_%d", i );
    sprintf ( Pname, "MNS3_%d", i );
  ...
  for ( int iN=0; iN<8; iN++ ) {
    char nameT[20];
    char nameN[20];
    sprintf ( nameT, "MTS3_%d", iN );
    sprintf ( nameN, "MNS3_%d", iN );
    double tot1[nbins]={0.};
    ...
    if ( iN==7 ) {  // Fix split histogram for Nt=9 && >9
      sprintf ( nameT, "MTS3_%d", iN+1 );
      sprintf ( nameN, "MNS3_%d", iN+1);
      for ( int i=0; i<8; i++ ) {
	MTtmp = dynamic_cast<TH1D*>(QCD[i]->Get(nameT));
	MNtmp = dynamic_cast<TH1D*>(QCD[i]->Get(nameN));
	for ( int ibin=1; ibin<=nbins; ibin++ ) {
  ...
  b->Print("./ps/TTMnS3.ps");
  
  // Close files
  // -----------
  TFile * File = new TFile ("TTMnS3.root","RECREATE");
  File->cd();
  for ( int i=0; i<8; i++ ) {
    MT[i]->Write();
    MN[i]->Write();
