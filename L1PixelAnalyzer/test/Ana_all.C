void Ana_all (TString pippo) 
{
  ...
  TFile * W[10];
  W[0] = new TFile ("./root/TDAna_W10_tk3.root");
  W[1] = new TFile ("./root/TDAna_W11_tk3.root");
  W[2] = new TFile ("./root/TDAna_W20_tk3.root");
  W[3] = new TFile ("./root/TDAna_W21_tk3.root");
  W[4] = new TFile ("./root/TDAna_W30_tk3.root");
  W[5] = new TFile ("./root/TDAna_W31_tk3.root");
  W[6] = new TFile ("./root/TDAna_W40_tk3.root");
  W[7] = new TFile ("./root/TDAna_W41_tk3.root");
  W[8] = new TFile ("./root/TDAna_W50_tk3.root");
  W[9] = new TFile ("./root/TDAna_W51_tk3.root");
  double Wxs[10] = { 9200., 250., 2500., 225., 590., 100., 125., 40., 85., 40. };
  double NW[10] = { 40000., 100530., 99523., 105255., 79000., 
  ...
  TTH->cd();
  double minx=MEtSigS->GetBinLowEdge(1);
  double maxx=50.*MEtSigS->GetBinWidth(1);
  ...
  for ( int i=0; i<8; i++ ) {
    ...
    for ( int ibin=1; ibin<=50; ibin++ ) {
      ...
      s2_totQCD[ibin-1]+=t*pow(QCDxs[i]/NQCD[i]*Lumfactor,2);
    }
  }
  double totTT[50]={0.};
  ...
  for ( int ibin=1; ibin<=50; ibin++ ) {
    ...
    // Until QCD errors are properly calculated...
    double grandtote= sqrt(s2_totTTH[ibin-1]+s2_totTT[ibin-1]+s2_totW[ibin-1]);    
